		...
		while (shoe.penetration() < shufflePenetration){
					...
					if (cid2BJnum(dealer.front()) == 1) {
						...
						if (stgtable.insurance(stgcount, breKO, inscount)) {
							...
							if (dealer.is_blackjack()) {
								stats.addReturn(bet * 1.5, RESISO, betcount, breKO);
								break;
							} else stats.addReturn(bet * 0, RESISX, betcount, breKO);
						}
					}
			...
			for (int h=0; h<MAXHAND; h++) {
				if (hand[h].is_active()) {
					...
					if (dealer.is_blackjack()) {
						if (hand[h].is_blackjack()) stats.addReturn(bet * 1, RESEBJ, betcount, breKO);
						else stats.addReturn(bet * 0, RESDBJ, betcount, breKO);
					} else if (hscr < BURST) {
						if (hand[h].is_surrender()) stats.addReturn(bet * 0.5, RESSUR, betcount, breKO);
						else if (hand[h].is_blackjack()) stats.addReturn(bet * 2.5, RESPBJ, betcount, breKO);
						else if (dscr >= BURST) stats.addReturn(bet * 2, RESDBS, betcount, breKO);
						else if (dscr < hscr) stats.addReturn(bet * 2, RESWIN, betcount, breKO);
						else if (dscr == hscr) stats.addReturn(bet * 1, RESEVN, betcount, breKO);
						else stats.addReturn(bet * 0, RESLOS, betcount, breKO);
					} else stats.addReturn(bet * 0, RESPBS, betcount, breKO);
				}
			}
			#ifdef DEBUG
				stats.show(breKO);
